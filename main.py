    ...
    def __init__(self):
        ...
        while True:
            ...
            if window == window_option and event == 'Operar':
                ...
                api.stop_win(float(((str(values['stop_win'])).replace(',','.'))))
                api.stop_loss(float((str(values['stop_loss'])).replace(',','.')))
                api.martingale(int(values['martingale']))
                api.multiplicador(float((str(values['multiplier'])).replace(',','.')))
                ...
                window_option.close()
                operate = True
                window_trading = Layouts.window_trading(api.get_type, api.get_balance, api.get_martingale, api.get_stop_win_complete, api.get_stop_loss_complete)
            ...
            if operate:
                ...
                window_trading['-status-'].update('Meta batida' if api.get_balance >= api.get_stop_win_complete else 'Stop atingido' if api.get_balance <= api.get_stop_loss_complete else f'Trabalhando ...' if api.get_balance > 0 else 'Sem fundos.')
                window_trading['-signal-'].update(f"{Actual_signal(file).get_next()}")
